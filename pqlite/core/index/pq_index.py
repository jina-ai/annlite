...
class PQIndex(BaseIndex):
    def __init__(self,
        ...
        super(PQIndex, self).__init__(pq_codec.n_subvectors, dtype=pq_codec.code_dtype, metric=metric, **kwargs)
    ...
    def search(self, x: np.ndarray, limit: int = 10, indices: Optional[np.ndarray] = None):
        _dim = x.shape[-1]
        assert _dim == self._pq_codec.dim, f'the query embedding dimension does not match with index dimension: {_dim} vs {self.dim}'
