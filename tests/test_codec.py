...
def build_pq_codec_online(build_data):
    Xt = build_data
    pq_codec_minibatch = PQCodec(dim=n_features, n_subvectors=n_subvectors, n_clusters=n_clusters)
    n_epochs = 3
    n_batch = 300
...
def test_partial_and_total_fit_same_codebook_shape(build_pq_codec, build_pq_codec_online):
