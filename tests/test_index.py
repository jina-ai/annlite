...
from annlite.hub import gen_docs
...
def test_remote_storage(annlite_index):
    os.environ['JINA_AUTH_TOKEN'] = 'ed17d158d95d3f53f60eed445d783c80'
    clear_hubble()

    docs = gen_docs(N)
    annlite_index.index(docs)
    annlite_index.backup(target_name='backup_docs')

    annlite_index.clear()
    annlite_index.restore(source_name='backup_docs')

    clear_hubble()
    status = annlite_index.stat
